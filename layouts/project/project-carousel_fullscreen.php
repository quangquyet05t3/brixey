    ...
    <div class="swiper-container horizontalscrollbar keyboard mousewheel" data-autoplay="true">
      <div class="swiper-wrapper">
				...
				foreach ($project_images as $key => $image) :
					...
					if (isset($image['video_url']) && !empty($image['video_url']) ) {
						$video_url = brixey_video_format_data( $image['video_url'] );
					} else {
						$video_url = '';
					}
				?>
        <div class="swiper-slide <?php echo esc_attr( $image['item_width'] ); ?>" style="background-image:url(<?php echo esc_url( $grid_image ); ?>);">
        <?php
        	if($video_url) {
        		echo '<div class="video-post"><a href="'. esc_url($video_url) .'" class="brxy-popup-video"></a></div>';
        	}
        	if(!$hide_zoom_icon){ ?>
      ...
      <div class="brxy-info-btn"><a href="javascript:void(0);"><i class="fa fa-info-circle" aria-hidden="true"></i> <?php echo esc_html__( 'Details', 'brixey' ); ?></a></div>
    </div>
