 ...
 */
require BRIXEY_ADMIN_PATH .'/cs-framework.php';
require BRIXEY_FRAMEWORK .'/theme-options/theme-options.php';
require BRIXEY_FRAMEWORK .'/theme-options/theme-customizer.php';
require BRIXEY_FRAMEWORK .'/theme-options/theme-metabox.php';
 ...
 */
function brixey_vt_cs_framework_override() {
  return 'inc/theme-options/theme-extend';
}
add_filter( 'cs_framework_override', 'brixey_vt_cs_framework_override' );

/**
 * Custom New Font Family
 */
if( ! function_exists( 'brixey_vt_custom_font_upload' ) ) {
  function brixey_vt_custom_font_upload( $db_value ) {
  ...
  add_action( 'cs_typography_family', 'brixey_vt_custom_font_upload' );
}

/**
 * Check Custom Font
 */
if ( ! function_exists( 'brixey_vt_custom_upload_font' ) ) {
  function brixey_vt_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'brixey_vt_registered_sidebars' ) ) {
  function brixey_vt_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'brixey_vt_google_fonts' ) ) {
  function brixey_vt_google_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'brixey-google-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'brixey_vt_get_typography' ) ) {
  function brixey_vt_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? brixey_vt_esc_string( $font['font']['variant'] ) : 400;
          $style   = brixey_vt_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. brixey_check_px( $font['size'] ) .';' : '';
